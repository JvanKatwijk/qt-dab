...
//	go for the filter
	...
	if (ret < 0)
	   theErrorLogger -> add ("Pluto", "setting filter failed");
//	and enable it
	filterButton	-> setText ("filter off");
	ret = ad9361_set_trx_fir_enable (get_ad9361_phy (ctx), 1);
	if (ret < 0)
	   theErrorLogger -> add ("Pluto", "enabling filter failed");
	connected	= true;
...
struct iio_channel *chn;
	...
	if (agcControl -> isChecked ()) {
	   ...
	   if (ret < 0) {
	      ...
	      theErrorLogger -> add ("Pluto",
	                        "could not change gain control to manual");
	      return;
	   }
	...
	if (ret < 0) {
	   state -> setText ("error in gain setting");
	   theErrorLogger -> add ("Pluto",
	            QString ("could not set hardware gain to " +
	                                       QString::number (newGain)));
	}
}
...
struct iio_channel *gain_channel;
	...
	if (agcControl -> isChecked ()) {
	   ...
	   if (ret < 0) {
	      theErrorLogger -> add ("Pluto", "setting agc failed");
	      state -> setText ("setting agc failed");
	      return;
	   }
	   else
	   ...
	   if (ret < 0) {
	      state -> setText ("error in gain setting");
	      theErrorLogger -> add ("Pluto", "error in gain setting");
	      return;
	   }
	   gainControl	-> show ();
	   ...
	   if (ret < 0) {
	      state -> setText ("error in gain setting");
	      QString t = "could not set hardware gain to " +
	                             QString::number (gainControl -> value ());
	      theErrorLogger -> add ("Pluto", t);
	   }
	}
}
...
//	settings are restored, now handle them
	ret = iio_channel_attr_write (gain_channel,
	                              ...
	                              agcControl -> isChecked () ?
	                                       "slow_attack" : "manual");
	if (ret < 0) {
	   theErrorLogger -> add ("Pluto", "error in setting agc");
	}
	else
	...
	ret = iio_channel_attr_write_longlong (lo_channel,
	                                       ...
	                                       rx_cfg. lo_hz);
	if (ret < 0) {
	   QString t = "cannot set local oscillator frequency to " +
	                                  QString::number (freq);
	   theErrorLogger -> add ("Pluto", t);
	   return false;
	}
