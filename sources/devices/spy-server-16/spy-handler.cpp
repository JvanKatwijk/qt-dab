...
bool	spyHandler::readBody	(uint8_t *buffer, uint32_t size) {
uint32_t	filler = 0;
	while (running. load ()) {
...
std::vector<uint32_t> p (1);
	fprintf (stderr, "request for start with streaming %d\n",
	                                  streaming. load () ? 1 : 0);
	if (!streaming. load ()) {
...
std::vector<uint32_t> p;
	fprintf (stderr, "request for stop with streaming %d\n",
	                                  streaming. load () ? 1 : 0);
	if (streaming. load ()) {
	   streaming. store (false);
	   p. push_back (0);
...
void	spyHandler::process_data (uint8_t *theBody, int length) {
	outB ->  putDataIntoBuffer ((int16_t *)theBody, length / 2);
	emit data_ready ();
}
