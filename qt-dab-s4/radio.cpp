...
//
//	Do we have a known device from a previous session
	inputDevice	= nullptr;
	h               =
...
//	fprintf (stderr, "%d %s\n", k, h. toUtf8 (). data ());
	if (k != -1) {
	   ...
	   inputDevice	= create_device (deviceSelector -> currentText());
	}
...
int	serviceOrder;
	...
	if (!handling_channel. load () || 
	             (my_ofdmHandler -> get_subCh_id (serviceName, SId) == -1))
	   return;
	(void)SId;
	serviceId ed;
	ed. name	= serviceName;
...
void	RadioInterface::show_label	(QString s) {
#ifdef	HAVE_PLUTO_RXTX
	if (streamerOut != nullptr)
...
void	RadioInterface::show_spectrum	(int32_t amount) {
	...
	my_spectrumViewer. showSpectrum (amount, tunedFrequency);
}
...
void	RadioInterface::show_clock_error	(int e) {
	if (!running. load ())
	   return;
...
QString channelName	= channel. channelName;
	...
	my_ofdmHandler -> start_dumping (rawDumper);
}
...
//	         return true;
	      ...
	      if (ad. defined && (serviceLabel -> text () == serviceName)) {
	         ...
	         my_presetHandler. addElement (pd. channel, pd. serviceName);
	         return true;
	      }
...
void    RadioInterface::handle_presetSelect (const QString &channel,
	...
	localSelect (channel, serviceName);
}
...
int	switchDelay;
	...
	if (theChannel == channel. channelName) {
	   ...
	   currentService. valid = false;
	   dabService s;
	   my_ofdmHandler -> get_parameters (service, &s. SId, &s. SCIds);
	   if (s. SId == 0) {
...
bool	found = false;
	...
	my_ofdmHandler -> get_parameters (selectedService, &s. SId, &s. SCIds);
...
QString presetButton_color =
           dabSettings -> value (PRESET_BUTTON + "_color",
                                                      "white"). toString ();
QString presetButton_font = 
           dabSettings -> value (PRESET_BUTTON + "_font",
                                                      "black"). toString ();
