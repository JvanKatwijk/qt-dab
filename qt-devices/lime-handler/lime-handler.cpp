...
lms_info_str_t limedevices [10];
	...
	theFilter. resize (currentDepth);
        ...
        if (Handle == nullptr) {
           throw (new lime_exception ("failed to open " +
	...
	if (!load_limeFunctions()) {
           ...
           dlclose (Handle);
#endif
           throw (new lime_exception ("could not load all required lib functions"));
        }
//
//      From here we have a library available
	int ndevs	= LMS_GetDeviceList (limedevices);
	if (ndevs == 0) {	// no devices found
	   throw (new lime_exception ("No lime device found"));
	}
	...
	if (res < 0) {	// some error
	   throw (new lime_exception ("failed to open device"));
	}

	res		= LMS_Init (theDevice);
	if (res < 0) {	// some error
	   LMS_Close (&theDevice);
	   throw (new lime_exception ("failed to initialize device"));
	}

	res		= LMS_GetNumChannels (theDevice, LMS_CH_RX);
	if (res < 0) {	// some error
	   LMS_Close (&theDevice);
	   throw (new lime_exception ("could not set number of channels"));
	}
	...
	if (res < 0) {	// some error
	   LMS_Close (theDevice);
	   throw (new lime_exception ("could not enable channels"));
	}

	res	= LMS_SetSampleRate (theDevice, 2048000.0, 0);
	if (res < 0) {
	   LMS_Close (theDevice);
	   throw (new lime_exception ("could not set samplerate"));
	}
...
//	default frequency
	...
	if (res < 0) {
	   LMS_Close (theDevice);
	   throw (new lime_exception ("could not set LO frequency"));
	}
	...
	if (res < 0) {
	   LMS_Close (theDevice);
	   throw (new lime_exception ("could not set bandwidth"));
	}
