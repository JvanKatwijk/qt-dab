...
static
std::complex<float> tmpBuffer [2 * FIFO_SIZE];
...
void	limeHandler::handle_Value	(int offset,
	...
	vfoFrequency	+= offset;
	LMS_SetLOFrequency (theDevice, LMS_CH_RX, 0, vfoFrequency);
...
int	amountRead	= 0;
	...
	while (running. load()) {
	   ...
	   if (res > 0) {
	      ...
	      base -> addSymbol (tmpBuffer, res / 2);
	      if (xmlDumping. load ())
	         xmlWriter -> add ((std::complex<int16_t> *)localBuffer,
	                                                      res / 2);
	      amountRead	+= res;
