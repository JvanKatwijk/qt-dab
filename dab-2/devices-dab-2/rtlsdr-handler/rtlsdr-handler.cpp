...
std::complex<float> localBuffer [READLEN_DEFAULT / 2];
	...
	for (uint32_t i = 0; i < len / 2; i ++) 
	   localBuffer [i] =
	              std::complex<float> (mapTable [buf [2 * i]],
	                                   mapTable [buf [2 * i + 1]]);

	(void)theStick -> base -> addSymbol (localBuffer, len / 2);
}
...
//	See what the saved values are and restore the GUI settings
	...
	product_display	-> setText (serial);
...
int32_t	r;
	...
	this -> rtlsdr_set_center_freq (device, freq);
