...
One of the things on my (endless) todo list was to look for an alternative for
the visterbi decoding. Viterbi decoding is used to map the soft bits, the result
of the interpretation of the FFT outout on the input blocks, to real hard bits.
Qt-DAB use(d|s) a decoder generated by the Spiral model, and while working OK,
the code was a little messy.

A change was made for the header only library of William Yang, which 
not only lead to much cleaner code, but it also provided compile time 
selection of cpu support (SSE4.1 and AVX2) for speeding up the (heavy)
computations in viterbi decoding.

Qt-DAB now contains **dynamic** selection of cpu support by either SSE4.1 or AVX2. If none of these can be founded, a scalar approach is selected.
(The main widget tells what was detected).
