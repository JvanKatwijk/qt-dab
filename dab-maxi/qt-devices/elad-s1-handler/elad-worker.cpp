...
//      (theRate / 1000) vs (2048000 / 1000)
        for (i = 0; i < DAB_RATE / 1000; i ++) {
           float inVal  = float (theRate / 1000);
           mapTable_int [i] =  int (floor (i * (inVal / (DAB_RATE / 1000))));
           mapTable_float [i] = i * (inVal / (DAB_RATE / 1000)) - mapTable_int [i];
        }
...
//	            fprintf (stderr, "start converting\n");
	            ...
	            for (j = 0; j < DAB_RATE / 1000; j ++) {
	               int16_t  inpBase       = mapTable_int [j];
                    ...
                    theBuffer -> putDataIntoBuffer (temp, DAB_RATE / 1000);
